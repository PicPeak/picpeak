 ...
 *     (LegacyContractResponse) runs exactly as before.
...
import {
  AlertTriangle, CheckCircle, Clock, Download, ShieldCheck, Upload, XCircle,
} from 'lucide-react';
...
export const ContractResponsePage: React.FC = () => {
  ...
  if (inviteQuery.isError) {
    ...
    if (status === 404 || status === 400) return <LegacyContractResponse token={token} />;
...
const LegacyContractResponse: React.FC<{ token: string }> = ({ token }) => {
  const { t } = useTranslation();
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => publicContractsService.get(token),
    retry: false,
  });
  useContractLanguage(data?.contract.language);

  const signMutation = useMutation({
    mutationFn: async () => publicContractsService.sign(token, {
      name: name.trim(),
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => publicContractsService.uploadSignedPdf(token, file),
    onSuccess: () => {
      setError(null);
      setUploadFile(null);
      queryClient.invalidateQueries({ queryKey: ['public-contract', token] });
      ...
      <div className={`mt-6 ${CARD}`}>
        {alreadySigned ? (
          <div className="py-2">
            ...
            {/* Streams whatever is the most authoritative copy on disk
                (signed_pdf_path when present, otherwise pdf_path). */}
            <div className="text-center mb-5">
              <a
            ...
            {/* Audit confirmation: every piece of evidence recorded, so the
                customer can keep it. Re-hashing the downloaded PDF and
                comparing against pdfSha256 proves the file wasn't altered. */}
            <details className="border-t border-neutral-200 dark:border-neutral-700 pt-4">
              ...
              <dl className="text-xs grid grid-cols-1 sm:grid-cols-[140px_1fr] gap-x-3 gap-y-1.5 font-mono">
                <dt className="text-neutral-500 dark:text-neutral-400">{t('publicContract.signed.contractNumber', 'Contract')}</dt>
                <dd>{c.contractNumber}</dd>
                {c.signedByCustomerAt && (
                  <>
                    <dt className="text-neutral-500 dark:text-neutral-400">{t('publicContract.signed.signedAt', 'Signed at')}</dt>
                    <dd>{new Date(c.signedByCustomerAt).toISOString()}</dd>
                  </>
                )}
                {c.signedCustomerIp && (
                  <>
                    <dt className="text-neutral-500 dark:text-neutral-400">{t('publicContract.signed.ipAddress', 'IP at signing')}</dt>
                    <dd>{c.signedCustomerIp}</dd>
                  </>
                )}
                {c.signedByAdminAt && (
                  <>
                    <dt className="text-neutral-500 dark:text-neutral-400">{t('publicContract.signed.counterSignedAt', 'Counter-signed at')}</dt>
                    <dd>{new Date(c.signedByAdminAt).toISOString()}</dd>
                  </>
                )}
                {c.signedPdfSha256 && (
                  <>
                    <dt className="text-neutral-500 dark:text-neutral-400">{t('publicContract.signed.signedSha', 'Signed PDF SHA-256')}</dt>
                    <dd className="break-all">{c.signedPdfSha256}</dd>
                  </>
                )}
                {c.pdfSha256 && (
                  <>
                    <dt className="text-neutral-500 dark:text-neutral-400">{t('publicContract.signed.unsignedSha', 'Original PDF SHA-256')}</dt>
          ...
          <>
            ...
            <form onSubmit={handleSign} className="space-y-3">
              ...
              <label className="flex items-start gap-2 text-sm text-neutral-700 dark:text-neutral-300">
                ...
                />
                <span>
                  {t('publicContract.acceptCheckbox', 'I have read this contract and agree to be bound by its terms.')}
                </span>
              </label>
            ...
            {/* Alternative: upload wet-signed PDF. Hidden when the admin has
                turned off the upload path in Settings → CRM behaviour. */}
