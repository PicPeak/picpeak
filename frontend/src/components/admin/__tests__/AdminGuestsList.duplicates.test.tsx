...
const guest = (id: number, name: string, email: string | null, duplicate_of: number[] = []) => ({
  id, name, email, duplicate_of,
  created_at: '2026-08-01T10:00:00Z', last_seen_at: '2026-08-02T10:00:00Z',
  email_verified_at: null, is_deleted: false,
  stats: { likes: 3, favorites: 1, comments: 0, ratings: 0, reactions: 0, color_labels: 0, distinct_photos: 3 },
});

const renderList = () => render(<AdminGuestsList eventId={7} eventName="Test" />, { wrapper });

describe('duplicate guests in the admin list (#1210)', () => {
  beforeEach(() => getEventGuests.mockReset());

  it('says how many entries look like returning visitors', async () => {
    getEventGuests.mockResolvedValue({
      guests: [guest(1, 'Tina', 'tina@example.com', [2]), guest(2, 'Tina', 'tina@example.com', [1])],
  ...
  it('badges the rows the banner is talking about', async () => {
    getEventGuests.mockResolvedValue({
      guests: [
        guest(1, 'Tina', 'tina@example.com', [2]),
        guest(2, 'Tina', 'tina@example.com', [1]),
