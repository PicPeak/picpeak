...
// This release has no generated_documents table; inbound_documents.file_path
// carries the several-values cases instead.
const doc = (p) => ({ file_path: p });
...
test('absolute paths under the storage root become relative; everything else stays; down restores them', async () => {
  ...
  const values = {
    ...
    legacyCwd: 'storage/business-docs/contract/2026/C-6.pdf',
    cwdRelative: path.relative(process.cwd(), path.join(root, 'business-docs', 'contract', '2026', 'C-5.pdf')),
  };
  const ids = {};
  for (const [key, value] of Object.entries(values)) {
    const [row] = await db('inbound_documents').insert(doc(value)).returning('id');
    ids[key] = row.id ?? row;
  }
  ...
  const read = async () => ({
    docs: Object.fromEntries(await Promise.all(Object.entries(ids).map(async ([k, id]) => [k, (await db('inbound_documents').where({ id }).first()).file_path]))),
  ...
  expect(down.docs.legacyCwd).toBe(values.legacyCwd);
